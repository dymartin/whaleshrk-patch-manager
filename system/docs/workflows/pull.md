...
2. Diff each `params.json` against `system/data/state/last-pushed/<song>.json`. The
...
`system/data/state/last-pushed/<song>.json` (the compiled `params.json`, byte-exact)
...
a song nobody touched. So `system/data/state/chains/<song>.json` records each
...
1. `samp_source` → folder via the decode table in
   ...
   lookup in `system/data/kits.yaml`.
