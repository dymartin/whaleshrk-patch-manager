...
system/                         non-musician-facing
  rig/                          Python application
  tests/                        tests grouped by application package
  fixtures/                     frozen ORHACK card used by tests
  docs/                         knowledge base
  media/
    samples/
    samples/loops/
    ...
    kits/tape/*.wav
  modules/
    <slug>@v<revision>.zip      Patchstorage upload, byte-identical
  data/                         machine-owned, committed
    ...
    state/
      last-pushed/
        ...
        .modules-lock-hash      hash of modules.lock as of last push
      chains/
        <song>.json             chain name -> letter binding
      hardware/
...
Only `songs/` is musician-facing. Source, tests, fixtures, documentation,
media, vendored modules, and generated state live under `system/`.
...
Musicians edit `songs/`; the CLI owns `system/data/` and `system/modules/`.
Maintainers place samples and kits in `system/media/`.

## Why `system/data/` is committed

`system/data/state/last-pushed/` is the drift baseline; `modules.lock` makes push
...
convention as every other `system/data/` JSON file. A recorded binding is
...
file and its `system/data/state/last-pushed/` entry.
