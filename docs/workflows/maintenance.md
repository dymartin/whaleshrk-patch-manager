...
Rewrites `name:` in the song file and the matching `.rig/state/chains/` binding
...
needs none. It **never** rewrites `.rig/state/last-pushed/` — the card is
...
version, so they surface as a reviewable `.rig/catalog/` diff.
